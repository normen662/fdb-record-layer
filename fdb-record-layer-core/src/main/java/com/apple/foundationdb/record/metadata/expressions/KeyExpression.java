...
public interface KeyExpression extends PlanHashable {
     ...
     * {@link com.apple.foundationdb.record.query.plan.temp.RelationalExpression}s
     * @param <S> a type that reflects the state that {@code visitor} uses
     * @param visitor a {@link ExpansionVisitor} that is created by the caller from a data structure that reflects the specific semantics
     *        of the key expression. Normally this visitor is directly created based on an index.
     * @return a new {@link GraphExpansion} representing the query graph equivalent of this key expression using the
     *         supplied visitor
     * @implNote Implementors should defer to the visitation methods in the supplied visitor rather than encoding actual
     *           logic in an overriding method.
